...
from tools.utils import batchify, get_batch, repackage_hidden
from optimizer import SGD, Adam, SGDF, AdaBound, AdaBelief, Yogi, MSVAG, RAdam, AdamW, Fromage
...
parser.add_argument('--save', type=str,  default='LSTM-PTB',
                    help='path to save the final model')
...
parser.add_argument('--when', nargs="+", type=int, default=[100,145],
...
args.save = args.save + '-niter-{}'.format(args.epochs) + '-optimizer-{}'.format(args.optimizer) + '-nlayers{}'.format(args.nlayers) + \
            '-lr{}'.format(args.lr) + '-betas-{}-{}'.format(args.beta1, args.beta2) +'-eps{}'.format(args.eps) + '-clip-{}'.format(args.clip)  \
            + '-wdecay{}-when-{}'.format(args.wdecay, args.when) + '-run{}'.format(args.seed) 
...
def model_save(fn):
    ...
    full_path = os.path.join(checkpoint_dir, fn)
...
def save_losses(train_losses, val_losses, fn):
    if not os.path.isdir(curve_dir):
        os.mkdir(curve_dir)
    full_path = os.path.join(curve_dir, fn)
    torch.save({
...
from tools.splitcross import SplitCrossEntropyLoss
criterion = None
...
if args.resume:
    ...
    if args.wdrop:
        from tools.weight_drop import WeightDrop
...
try:
    ...
    if args.optimizer == 'sgd':
        optimizer = SGD(params, lr=args.lr, weight_decay=args.wdecay)
    if args.optimizer == 'sgdf':
        optimizer = SGDF(params, lr=args.lr, betas=(args.beta1, args.beta2), eps=args.eps, weight_decay=args.wdecay)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        #print(train_losses)
        save_losses(train_losses, val_losses, args.save)
        # torch.save({'train_loss': train_losses, 'test_loss': val_losses},
        #            os.path.join(curve_dir, args.save))
