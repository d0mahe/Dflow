...
class Attention(nn.Module):
    ...
    def forward(self, x, rel_pos_bias=None):
        ...
        # Ensure qkv_bias has the same dtype as x        
        qkv_bias = qkv_bias.type_as(x) if qkv_bias is not None else None
        #qkv = F.linear(input=x, weight=self.qkv.weight, bias=qkv_bias.type(x.dtype))
        qkv = F.linear(input=x, weight=self.qkv.weight.type_as(x), bias=qkv_bias)
        ...
        if self.relative_position_bias_table is not None:
            relative_position_bias = self.relative_position_bias_table[self.relative_position_index.view(-1)].view(
                ...
                self.window_size[0] * self.window_size[1] + self.num_extra_tokens, -1)
        ...
        x = self.proj(x.type_as(self.proj.weight))
        x = self.proj_drop(x)
        return x
...
class ViT(nn.Module):
    ...
    def __init__(self, image_size=224, patch_size=16, in_channels=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 learn_sigma=False, use_fp16=False, drop_label_prob=0.0, **kwargs):  #classifier_free_scale=1.0,
        super().__init__()
...
def ViT_S(image_size, patch_size, num_classes, in_channels, learn_sigma, drop_rate, drop_label_prob, **kwargs):
    return ViT(image_size=image_size, patch_size=patch_size, embed_dim=512, depth=13, num_heads=4, mlp_ratio=4, num_classes=num_classes, 
               in_channels=in_channels, learn_sigma=learn_sigma, drop_rate=drop_rate, drop_label_prob=drop_label_prob, **kwargs)

def ViT_B(image_size, patch_size, num_classes, in_channels, learn_sigma, drop_rate, drop_label_prob, **kwargs):
    return ViT(image_size=image_size, patch_size=patch_size, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, num_classes=num_classes, 
               in_channels=in_channels, learn_sigma=learn_sigma, drop_rate=drop_rate, drop_label_prob=drop_label_prob, **kwargs)

def ViT_L(image_size, patch_size, num_classes, in_channels, learn_sigma, drop_rate, drop_label_prob, **kwargs):
    return ViT(image_size=image_size, patch_size=patch_size, embed_dim=1024, depth=21, num_heads=16, mlp_ratio=4, num_classes=num_classes, 
               in_channels=in_channels, learn_sigma=learn_sigma, drop_rate=drop_rate, drop_label_prob=drop_label_prob,  **kwargs)

def ViT_XL(image_size, patch_size, num_classes, in_channels, learn_sigma, drop_rate, drop_label_prob, **kwargs):
    return ViT(image_size=image_size, patch_size=patch_size, embed_dim=1152, depth=28, num_heads=16, mlp_ratio=4, num_classes=num_classes, 
               in_channels=in_channels, learn_sigma=learn_sigma, drop_rate=drop_rate, drop_label_prob=drop_label_prob,  **kwargs)
...
if __name__ == '__main__':
    vit_model = ViT_S(image_size=32, patch_size=4, num_classes=1000).cuda()
